...
func Test_Delete(t *testing.T) {
	...
	withContext(t, models, func(q Querier, c *hades.Context) {
		...
		wtest.Must(t, c.Save(q, stories))

		count, err = c.Count(q, &Story{}, builder.NewCond())
		wtest.Must(t, err)
		assert.EqualValues(t, 3, count)

		err = c.Delete(q, &Story{}, builder.NewCond())
		assert.Error(t, err, "must refuse to delete with empty cond")
		assert.Contains(t, err.Error(), "refusing to blindly")

		err = c.Delete(q, &Story{}, builder.Eq{"id": 1})
		wtest.Must(t, err)
		count, err = c.Count(q, &Story{}, builder.NewCond())
		wtest.Must(t, err)
		assert.EqualValues(t, 2, count)

		err = c.Delete(q, &Story{}, builder.Expr("1"))
