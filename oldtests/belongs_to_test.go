...
func Test_BelongsTo(t *testing.T) {
	...
	withContext(t, models, func(q Querier, c *hades.Context) {
		...
		wtest.Must(t, c.Save(q, someFate))
		...
		wtest.Must(t, c.Save(q, lea))

		t.Log("Preloading lea")
		c.Preload(q, lea, hades.Assoc("Fate"))
	...
	withContext(t, models, func(q Querier, c *hades.Context) {
		...
		wtest.Must(t, c.Save(q, lea, hades.Assoc("Fate")))

		fate := &Fate{}
		found, err := c.SelectOne(q, fate, builder.Eq{"id": 421})
		wtest.Must(t, err)
		assert.True(t, found)
	...
	withContext(t, models, func(q Querier, c *hades.Context) {
		...
		wtest.Must(t, c.Save(q, fate))

		human := &Human{
			ID:     6,
			FateID: 3,
		}
		wtest.Must(t, c.Save(q, human))

		joke := &Joke{
			ID:      "neuf",
			HumanID: 6,
		}
		wtest.Must(t, c.Save(q, joke))

		c.Preload(q, joke, hades.Assoc("Human", hades.Assoc("Fate")))
