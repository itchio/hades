...
func Test_SquashedInsert(t *testing.T) {
	...
	withContext(t, models, func(q Querier, c *hades.Context) {
		...
		wtest.Must(t, c.Insert(q, scope, val))

		bb := &Bone{}
		wtest.Must(t, c.ExecRaw(q, "SELECT * FROM bones", func(stmt *sqlite.Stmt) error {
...
func Test_SquashedFull(t *testing.T) {
	...
	withContext(t, models, func(q Querier, c *hades.Context) {
		...
		wtest.Must(t, c.Save(q, fu, hades.Assoc("Games")))

		u := &FakeUser{}

		found, err := c.SelectOne(q, u, builder.NewCond())
		wtest.Must(t, err)
		assert.True(t, found)

		assert.EqualValues(t, 15, u.ID)

		wtest.Must(t, c.Preload(q, u,
